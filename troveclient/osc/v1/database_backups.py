...
class CreateDatabaseBackup(command.ShowOne):
    ...
    def get_parser(self, prog_name):
        ...
        parser.add_argument(
            '--storage-driver',
            help=_('The storage driver - that being use to save backup data. '
                   'Current, valid values are: swift, cinder, s3, ... '
                   'it being depend by trove support. '
                   'Maybe conflict with other options')
        )
        return parser
